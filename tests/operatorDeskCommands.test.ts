...
describe("operator desk command and diagnostic summaries", () => {
  ...
  it("surfaces media review commands in operator desk output", async () => {
    ...
    expect(output).toContain(
      `- Draft render: pass (8s, intro -> scene -> outro, source frames intro:2/outro:2, frame cadence intro#1=1s assets/intro/frames/intro_frame_00.jpg; intro#2=1s assets/intro/frames/intro_frame_01.jpg; outro#1=1.5s assets/outro/frames/outro_frame_00.jpg; outro#2=1.5s assets/outro/frames/outro_frame_01.jpg, voiceover local-piper production candidate, approval approval_render_status, ffprobe 1280x720 audio) | Local playback path: runs/${run.runId}/production/render/draft.mp4 | Review command: pnpm producer review render --run ${run.runId}`,
    );
