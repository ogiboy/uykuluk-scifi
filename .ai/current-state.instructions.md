...
- Project-local capability routing, resumable `.ai/checkpoints/`, and the conventional-commit
  ...
  and a release gate that waits for the triggering SHA's CircleCI status. GitHub App event filtering
  keeps PR revisions to one full CircleCI workflow while preserving `main` and explicit API runs.
