...
- Harden manual analytics feedback with import edge cases, richer comparisons, and recommendations
  while keeping data operator-provided and local-only. Run-linked summaries, unmapped-record
  visibility, and the basic read-only Studio analytics overview are implemented.
