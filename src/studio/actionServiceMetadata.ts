...
export const studioMutationActionIds = [
  ...
  "localModels.prepare", "localModels.execute",
  ...
  "visuals.prepare", "visuals.plan-hosted", "visuals.generate-hosted", "visuals.generate-local", "visuals.import", "visuals.decide", "visuals.regenerate", "visuals.activate-revision",
  "soundtrack.prepare", "soundtrack.import", "soundtrack.configure", "soundtrack.analyze", "soundtrack.decide",
...
const studioMutationServiceRows = [
  ...
  ["localModels.prepare", readyForCli, "pnpm producer local-model prepare --file <temp_json>", "prepareLocalModelOperation", "src/localModels/localModelReadiness.ts", "Persist an exact no-cost local-model time and disk preflight before any heavy local operation."],
  ["localModels.execute", readyForCli, "pnpm producer local-model execute --file <temp_json>", "executeApprovedLocalModelOperation", "src/localModels/localModelReadiness.ts", "Queue only the exact operator-approved local-model setup, verification, or smoke operation."],
  ...
  ["visuals.plan-hosted", readyForCli, "pnpm producer visuals plan-hosted --run <run_id> --scenes <scene_indexes> --purpose <purpose>", "prepareHostedVisualGenerationPlan", "src/stages/visuals/visualGenerationPlanStore.ts", "Bind selected visual beats to one exact hosted-provider plan before cost approval."],
  ...
  ["visuals.generate-local", readyForCli, "pnpm producer visuals generate-local --run <run_id> --scenes <scene_indexes> --expected-manifest-digest <sha256> --expected-active-revisions-file <temp_json>", "generateLocalVisuals", "src/stages/visuals/localVisualGeneration.ts", "Generate sequential MFLUX revisions for selected scenes after local-model readiness."],
  ...
  ["visuals.activate-revision", readyForCli, "pnpm producer visuals activate-revision --run <run_id> --scene <scene_index> --revision <revision> --expected-manifest-digest <sha256> --expected-active-revisions-file <temp_json>", "activateVisualRevision", "src/stages/visuals/visualRevisionActivation.ts", "Make one reviewed historical scene revision canonical and reopen its decision."],
  ["soundtrack.prepare", readyForCli, "pnpm producer soundtrack prepare --run <run_id>", "prepareVoiceOnlySoundtrack", "src/stages/soundtrack/soundtrackService.ts", "Create the initial voice-only soundtrack manifest from current reviewed voice evidence."],
  ["soundtrack.import", readyForCli, "pnpm producer soundtrack import --run <run_id> --asset <asset_id> --role <music_or_sfx> --file <temp_file> --provenance-file <temp_json> --expected-manifest-digest <sha256> --expected-revision <revision>", "importSoundtrackAudio", "src/stages/soundtrack/soundtrackService.ts", "Import one bounded local music or SFX file with rights provenance."],
  ["soundtrack.configure", readyForCli, "pnpm producer soundtrack configure --run <run_id> --file <temp_json> --expected-manifest-digest <sha256> --expected-revision <revision>", "configureSoundtrackMix", "src/stages/soundtrack/soundtrackService.ts", "Configure selected music and timed SFX cues, invalidating prior analysis."],
  ["soundtrack.analyze", readyForCli, "pnpm producer soundtrack analyze --run <run_id> --expected-manifest-digest <sha256> --expected-revision <revision>", "analyzeSoundtrackLoudness", "src/stages/soundtrack/soundtrackService.ts", "Run bounded local FFmpeg loudness analysis without provider execution."],
  ["soundtrack.decide", readyForCli, "pnpm producer soundtrack decide --run <run_id> --decision <approved_or_rejected> --reviewed-by <operator> --notes <notes> --expected-manifest-digest <sha256> --expected-revision <revision>", "decideSoundtrack", "src/stages/soundtrack/soundtrackService.ts", "Record an attributed soundtrack approval or rejection bound to its current revision."],
