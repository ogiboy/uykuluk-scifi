...
export function validateIdeaQuality(
  ...
  const channelAttributionIssue = invalidChannelAttributionIssue(idea.fit);
  if (channelAttributionIssue) {
    return channelAttributionIssue;
  }
...
function invalidChannelAttributionIssue(
  fit: string,
): { path: string[]; message: string } | undefined {
  const words = ideaSignature(fit).split(" ").filter(Boolean);
  const invalidIndex = words.findIndex((word, index) => {
    if (index === 0 || !misattributedChannelForms.has(word)) {
      return false;
    }
    return !allowedChannelQualifiers.has(words[index - 1]);
  });
  return invalidIndex >= 0
    ? {
        path: ["fit"],
        message:
          "Only UykulukSciFi may be described as the channel; do not turn a topic or title into a channel name.",
      }
    : undefined;
}
...
const misattributedChannelForms = new Set(["kanalın", "kanalını", "kanalının"]);
const allowedChannelQualifiers = new Set(["bu", "uykulukscifi"]);
