...
export type DraftRenderTiming = {
  ...
  sceneAudioDurationSeconds: number;
  outroDurationSeconds: number;
  totalDurationSeconds: number;
};

/**
 * Computes the complete draft duration, keeping bookends outside the voiceover window unless an
 * explicit review-duration cap requires the whole composition to be shortened.
 *
 * @param renderPlan - The render plan that owns intro and outro durations.
 * @param voiceoverDurationSeconds - The validated spoken-audio duration.
 * @param maxDurationSeconds - Optional cap for intentionally short local review renders.
 * @returns The total target duration for the complete draft composition.
 */
export function draftRenderTargetDuration(
  renderPlan: RenderPlan,
  voiceoverDurationSeconds: number,
  maxDurationSeconds?: number,
): number {
  const bookendDuration = renderPlan.bookends
    ? renderPlan.bookends.intro.durationSeconds + renderPlan.bookends.outro.durationSeconds
    : 0;
  return clampRenderDuration(voiceoverDurationSeconds + bookendDuration, maxDurationSeconds);
}

/**
 * Summarizes the timeline windows used to align video, voiceover, subtitles, and outro silence.
 *
 * @param timeline - The complete draft-render timeline.
 * @returns Rounded timing windows whose sum is the rendered output duration.
 */
export function summarizeDraftRenderTimeline(timeline: DraftRenderTimeline): DraftRenderTiming {
  const durationFor = (segment: DraftRenderTimelineSegment): number =>
    roundSeconds(
      timeline
        .filter((item) => item.segment === segment)
        .reduce((total, item) => total + item.durationSeconds, 0),
    );
  const introDurationSeconds = durationFor("intro");
  const sceneAudioDurationSeconds = durationFor("scene");
  const outroDurationSeconds = durationFor("outro");
  return {
    introDurationSeconds,
    sceneAudioDurationSeconds,
    outroDurationSeconds,
    totalDurationSeconds: roundSeconds(
      introDurationSeconds + sceneAudioDurationSeconds + outroDurationSeconds,
    ),
  };
}
