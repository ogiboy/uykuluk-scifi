/** Canonical local render-decision values and operator command templates. */
...
/**
 * Builds the guarded command that archives a non-accepted draft before a fresh render approval.
 *
 * @param runId - The rendered run whose current draft must be replaced.
 * @returns A copy-pasteable render-revision command.
 */
export function renderRevisionCommand(runId: string): string {
  return renderShellCommand("pnpm", ["producer", "revise", "render", "--run", runId]);
}
