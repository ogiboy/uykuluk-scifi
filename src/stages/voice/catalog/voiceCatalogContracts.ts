...
const candidateSharingSchema = z.strictObject({
  ...
  disableAtUnix: z.int().nonnegative().optional(),
...
export const subscriptionSnapshotSchema = z.strictObject({
  ...
  characterLimit: z.int().nonnegative(),
...
const modelSnapshotSchema = z.strictObject({
  ...
  maxCharactersRequestFreeUser: z.int().positive().max(100_000).optional(),
  maxCharactersRequestSubscribedUser: z.int().positive().max(100_000).optional(),
...
export const voiceCatalogProviderResultSchema = z.strictObject({
  ...
  sourceVoiceCount: z.int().nonnegative().max(10_000),
  rejectedVoiceCount: z.int().nonnegative().max(10_000),
