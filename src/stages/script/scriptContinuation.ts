...
export async function applyLongFormContinuations(input: ScriptContinuationInput): Promise<void> {
  ...
  for (const chunk of scriptContinuationChunks) {
    ...
    const missingWords = longFormWordFloor - countSpokenNarrationWords(currentScript);
    if (missingWords <= 0) {
      return;
    }
...
export function assertLongFormWordFloor(script: string): void {
  const wordCount = countSpokenNarrationWords(script);
...
export function renderScriptContinuationPrompt(
  ...
  const target = continuationTarget(options.missingWords, options.chunk.index);
  return [
    ...
    `Current missing spoken-narration floor estimate: ${Math.max(0, options.missingWords)} words.`,
    `Target length: ${target.minimumWords}-${target.maximumWords} Turkish spoken narration words; visual directions do not count toward the floor.`,
    `Hard limit: ${target.maximumCharacters} characters. Stop early rather than exceeding this limit.`,
    "Write 12-16 complete Turkish sentences total; at least 10 should be Anlatıcı: sentences and at most 2 should be Görsel: directions.",
    "Keep complete sentences, calm cinematic pacing, scientific caution, and Turkish production labels only.",
    "Spell production labels exactly as Anlatıcı: and Görsel:. Write labels as plain text without backticks.",
...
export function scriptContinuationTokenCap(totalScriptCap: number): number {
  return Math.min(totalScriptCap, Math.max(1000, Math.ceil(totalScriptCap / 4)));
}

export function continuationTarget(
  missingWords: number,
  chunkIndex: ScriptContinuationChunk["index"],
): { maximumCharacters: number; maximumWords: number; minimumWords: number } {
  const remainingChunks = scriptContinuationChunks.length - chunkIndex + 1;
  const bufferedShare = Math.ceil((Math.max(0, missingWords) / remainingChunks) * 1.35);
  const minimumWords = Math.min(420, Math.max(220, bufferedShare));
  return { minimumWords, maximumWords: Math.min(480, minimumWords + 60), maximumCharacters: 3600 };
}
