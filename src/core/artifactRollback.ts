...
/** Captures exact run-artifact bytes so a failed multi-file mutation can restore them. */
export async function captureRunArtifactRollback(
  runId: string,
  stage: string,
  ...
  return async (failure) => {
    for (const snapshot of snapshots) {
      ...
      if (snapshot.bytes) await writeBinaryFile(target, snapshot.bytes);
